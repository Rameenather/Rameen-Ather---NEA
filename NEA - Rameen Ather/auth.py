    ...
    if request.method == 'POST':
        ...
        if user:
            if check_password_hash(user.password, password):
                ...
                connected_user[request.sid] = user
                conn = sqlite3.connect('database.db')
                cursor = conn.cursor()

                cursor.execute('''SELECT * FROM messages WHERE recipient_id=?''', (user['id'],))
                messages = cursor.fetchall()
                for message in messages:
                    request.namespace.emit('message', message['message'])
        
                # Delete the messages from the database
                #cursor.execute('''DELETE FROM messages WHERE recipient_id=?''', (user['id'],))
            else:
